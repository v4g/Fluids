...
void MyWorld::initialize(int _numCells, double _timeStep, double _diffCoef, double _viscCoef) {
	...
	mDensityRGB[0] = new double[size];
	mDensityRGB[1] = new double[size];
	mDensityRGB[2] = new double[size];
...
void MyWorld::simulate() {
	...
	for(int i = 0; i < 3; i++)
		densityStep(mDensityRGB[i], mPreDensity);
	externalForces();
}
...
void MyWorld::velocityStep(double *_u, double *_v, double *_u0, double *_v0) {
	...
	advectVelocity(_u, _u0, _u0, _v0);
	advectVelocity(_v, _v0, _u0, _v0);
	project(_u, _v, _u0, _v0);
}
...
void MyWorld::advectVelocity(double *_u, double *_v, double *_u0, double *_v0) {
	...
	for (int i = 1; i <= mNumCells; i++) {
		for (int j = 1; j <= mNumCells; j++) {
			...
			double t1 = y - j0;
			double t0 = 1 - t1;
			_u[IX(i, j)] = s0 * (t0 * _v[IX(i0, j0)] + t1 * _v[IX(i0, j1)]) + s1 * (t0 * _v[IX(i1, j0)] + t1 * _v[IX(i1, j1)]);
		}
	}
	setVelocityBoundary(_u, _v);
}

void MyWorld::project(double *u, double *v, double *u0, double *v0) {
   // TODO: Add projection code here
	float h = 1.0 / mNumCells;
	for (int i = 1; i <= mNumCells; i++) {
		for (int j = 1; j <= mNumCells; j++) {
	...
	for (int i = 1; i <= mNumCells; i++) {
		for (int j = 1; j <= mNumCells; j++) {
			u[IX(i, j)] -= 0.5*(u0[IX(i + 1, j)] - u0[IX(i - 1, j)]) / h;
			v[IX(i, j)] -= 0.5*(u0[IX(i, j + 1)] - u0[IX(i, j - 1)]) / h;
		}
	}
	setVelocityBoundary(u,v);
}
