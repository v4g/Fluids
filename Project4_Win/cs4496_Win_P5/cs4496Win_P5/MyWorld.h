...
class MyWorld {
 public:
	...
	void getDensityRGB(int _index, double *dense) {
		dense[0] = mDensityRGB[0][_index];
		dense[1] = mDensityRGB[1][_index];
		dense[2] = mDensityRGB[2][_index];
	...
	void setDensityRGB(int _index, int _i, int _j, double _source) { 
		mDensityRGB[_index][IX(_i, _j)] += mTimeStep * _source; 
	}
