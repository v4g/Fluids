...
void myGlutKeyboard(unsigned char key, int x, int y) {
    switch (key) {
		...
		case 'v': // toggle between velocity view and density view
			...
			break;
		case 'd': // toggle between velocity view and density view
			currField = (++currField)%3;
			break;
		default:
            break;
    }
  ...
  mouse_down = (state == GLUT_DOWN);
    ...
    if(mouse_down){
        ...
        if (button == GLUT_LEFT_BUTTON) {
            leftClick = true;
			mySimulator.setDensityRGB(currField, i, j, 100.0);
...
void drawDensity() {
    double h = 1.0 / mySimulator.getNumCells();
	glBegin(GL_QUADS);
	double * da00 = new double[3];
	double * da01 = new double[3];
	double * da10 = new double[3];
	double * da11 = new double[3];
	...
	for (int i = 0; i <= mySimulator.getNumCells(); i++) {
		...
		for (int j = 0; j <= mySimulator.getNumCells(); j++) {	
				...
				mySimulator.getDensityRGB(IX(i, j),da00);
				mySimulator.getDensityRGB(IX(i, j + 1), da01);
				mySimulator.getDensityRGB(IX(i + 1, j), da10);
				mySimulator.getDensityRGB(IX(i + 1, j + 1), da11);
				glColor3d(da00[0], da00[1], da00[2]);
				glVertex3f(x, y, 0);
				glColor3d(da10[0], da10[1], da10[2]);
				glVertex3f(x + h, y, 0);
				glColor3d(da11[0], da11[1], da11[2]);
				glVertex3f(x + h, y + h, 0);
				glColor3d(da01[0], da01[1], da01[2]);
				glVertex3f(x, y + h, 0);
				}
			}
	glEnd();
